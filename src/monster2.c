...
void pack_choose_ai(int m_idx)
{
    ...
    if (m_ptr->pack_idx)
    {
        ...
        /* Reverse animal vs Sentient pack AI - from Sil */
        /* Empty-minded creatures also behave instinctively */
        else if ((r_ptr->flags3 & RF3_ANIMAL) || (r_ptr->flags2 & RF2_EMPTY_MIND))
        {
            switch (randint1(10))
            {
...
void monster_desc(char *desc, monster_type *m_ptr, int mode)
{
    ...
    #endif
    if ((p_ptr->wizard || TRUE) && (m_ptr->mpower != 1000))
    {
