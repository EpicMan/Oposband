...
byte value_check_aux1(object_type *o_ptr, bool remote)
{
    ...
    /* Average weapons, armor, ammo and lites can all be IDed since the player will
     * (after playing a few games) know all their properties just by knowing the object
     * is average
     */
    if (object_is_weapon_armour_ammo(o_ptr) || object_is_lite(o_ptr))
        obj_identify(o_ptr);
...
//        msg_format("Redesigning mystery cave - new_game %s, cave ready %s", new_game ? "Yes" : "No", mystery_cave_ready ? "Yes" : "No");
    ...
    if (new_game)
    {
        ...
        /* Chaos warriors and logrus masters get a patron effect at lvl 1 */
        if (worships_chaos())
        {
            chaos_choose_effect(PATRON_LEVEL_UP);
        }
    }
